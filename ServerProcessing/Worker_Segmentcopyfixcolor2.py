...
# --- BƯỚC 3: TÔ MÀU VÀ LƯU FILE ---
        ...
        # 🚨 ĐOẠN CẦN SỬA ĐÂY NHÉ: Đảm bảo đổi tên file đúng mọi trường hợp!
        if npy_path.endswith('.nii'):
            result_path = npy_path.replace('.nii', '_Segmented_Colored.npy')
        elif npy_path.endswith('.nii.gz'):
            result_path = npy_path.replace('.nii.gz', '_Segmented_Colored.npy')
        else:
