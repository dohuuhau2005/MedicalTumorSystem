...
# --- BƯỚC 2: CHẠY AI ---
        slices_2d = np.load(npy_path,allow_pickle=True)
        print(f"⚙️ Chạy inference cho {len(slices_2d)} lát cắt...")
        
        # 🚨 THÊM ĐOẠN NÀY ĐỂ ÉP CHUẨN HÓA ẢNH VỀ DẢI [0, 1] TRƯỚC KHI ĐƯA VÀO AI
        slices_2d = slices_2d.astype(np.float32)
        # Công thức (X - Min) / (Max - Min). Thêm 1e-8 để chống lỗi chia cho 0
        slices_2d = (slices_2d - np.min(slices_2d)) / (np.max(slices_2d) - np.min(slices_2d) + 1e-8)

        # AI trả về 1 mảng duy nhất có 3 kênh màu ở cuối: Shape (batch, 256, 256, 3)
