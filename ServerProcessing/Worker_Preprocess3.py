...
def normalize(img):
    ...
    if np.sum(mask) > 0:
        ...
        # 🚨 FIX LỖI: Phải ép viền đen bên ngoài về lại số 0
        img_norm[~mask] = 0.0 
        return img_norm
    return img
...
def volume_to_slices_realtime(volume, size=256):
    ...
    for i in range(volume.shape[2]):
        ...
        # 🚨 FIX LỖI: Xoay ảnh dựng đứng sọ lên để AI dễ nhìn
...
# --- BƯỚC 2: CẮT ẢNH 3D SANG 2D ---
        ...
        # 🚨 FIX LỖI TÊN FILE: Đổi đuôi linh hoạt cho cả .nii và .nii.gz
        if file_path.endswith('.nii'):
            out_path = file_path.replace('.nii', '_2D_ready.npy')
        elif file_path.endswith('.nii.gz'):
            out_path = file_path.replace('.nii.gz', '_2D_ready.npy')
        else:
            out_path = file_path + "_2D_ready.npy"
