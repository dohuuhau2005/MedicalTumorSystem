...
def apply_color_mask(original_slice, wt_pred, tc_pred, et_pred):
    # 1. TRỊ DỨT ĐIỂM LỖI 3 KÊNH ÂM CỦA Z-SCORE
    # Lấy ra 1 kênh xám nguyên thủy để xử lý (bỏ 2 kênh ảo đi)
    img_gray = original_slice[:, :, 0]
    
    # 2. GỌT NHIỄU SÁNG & TỐI (Giống vmax của matplotlib)
    p1, p99 = np.percentile(img_gray, (1, 99))
    img_clipped = np.clip(img_gray, p1, p99)
    
    # 3. CHUẨN HÓA VỀ ĐÚNG KHUNG 0 - 255 (Bằng công thức toán học)
    if p99 - p1 > 0:
        img_norm = (img_clipped - p1) / (p99 - p1)
    else:
        img_norm = img_clipped - p1 # Tránh lỗi chia cho 0
        
    img_8u = (img_norm * 255).astype(np.uint8)
    
    # 4. NHÂN BẢN LẠI THÀNH RGB (Để có chỗ tô màu Xanh/Đỏ/Vàng)
    ...
    color_mask[tc_mask == 1] = [255, 0, 0]   # TC: Đỏ
    color_mask[et_mask == 1] = [255, 255, 0] # ET: Vàng

    # Trộn ảnh gốc với mặt nạ (0.7 là độ nét não, 0.5 là độ đậm của u)
    blended_img = cv2.addWeighted(orig_rgb, 0.7, color_mask, 0.5, 0)
    return blended_img
